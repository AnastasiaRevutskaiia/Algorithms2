...
public class BoggleGame extends JFrame {
    ...
    public BoggleGame(int rows, int cols) {
        ...
        try {
            ...
            In in1 = new In(Paths.get(getClass().getClassLoader().getResource("assignment4/dictionary-shakespeare.txt").toURI()).toFile());
            shakespeareDictionary = new SET<String>();
            for (String s : in1.readAllStrings())
                shakespeareDictionary.add(s);

            // all words in shakespeare
            In in2 = new In(Paths.get(getClass().getClassLoader().getResource("assignment4/dictionary-nursery.txt").toURI()).toFile());
            nurseryDictionary = new SET<String>();
            for (String s : in2.readAllStrings())
                nurseryDictionary.add(s);

            // about 20K common words
            In in3 = new In(Paths.get(getClass().getClassLoader().getResource("assignment4/dictionary-common.txt")
                    .toURI()).toFile());
            ...
            In in4 = new In(Paths.get(getClass().getClassLoader().getResource("assignment4/dictionary-algs4.txt")
                    .toURI()).toFile());
            ...
            // dictionary
            in = new In(Paths.get(getClass().getClassLoader().getResource("assignment4/dictionary-yawl.txt")
                    .toURI()).toFile());
            ...
            public void run() {
                ...
                if (args.length == 0) {
                    ...
                    try {
                        ...
                        System.err.println("Usage: java assignment4.BoggleGame " +
                                           "\nor:    java assignment4.BoggleGame [rows]" +
                                           "\nor:    java assignment4.BoggleGame [rows] [cols]");
                        System.exit(1);
                    }
                }
                ...
                if (rows <= 0 || cols <= 0) {
                    throw new java.lang.IllegalArgumentException("Rows and columns must be positive" + 
                                                                 "\nUsage: java assignment4.BoggleGame " +
                                                                 "\nor:    java assignment4.BoggleGame [rows]" +
                                                                 "\nor:    java assignment4.BoggleGame [rows] [cols]");
                }
